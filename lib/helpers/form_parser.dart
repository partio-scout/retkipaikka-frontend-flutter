...
class FormParser{
  ...
  static Map<String,dynamic>? afterFormSubmit(Map<String,dynamic> rawFormData,BuildContext context){
    ...
    AbstractFilter? municipality = state.allMunicipalities.firstWhereOrNull((element) => element.name == dataCpy["location_area"]);
    int? municipalityId;
    int regionId = -1;
    if(municipality != null){
      ...
      AbstractFilter? region = state.allRegions.firstWhereOrNull((element) => element.name == dataCpy["location_area"]);
      if(region != null){
        regionId = region.id;
      }   
    }
  ...
  static List<XFile> imageStringsToXFile(List<String> images, String locationId){
  
    List<XFile> imgs = [];

    for (var image in images) {

      var path = BaseApi.baseUrl + "/Images/" + locationId + "/download/" + image;
